$(document).ready(function () {
  ...
  var categoryImages = [
    ...
    },
    {
      ...
      url: 'ArtsCultureHumanities2.jpg',
      catNum: '2',
      photoCat: 'Arts',
    },
    {
    ...
    },
    {
      ...
      url: 'Environment4.jpg',
      catNum: '4',
      photoCat: 'Environment',
    },
    { category: 'Health', url: 'Health5.jpg', catNum: '5', photoCat: 'Health' },
    {
      category: 'Human Services',
      url: 'HumanServices6.jpg',
      catNum: '6',
      photoCat: 'Humanity',
    },
    {
      category: 'Human Civil Rights',
      url: 'HumanCivilRights8.jpg',
      catNum: '8',
      photoCat: 'Civil+Rights',
    },
    {
      category: 'Religion',
      url: 'Religion9.jpg',
    ...
    {
      category: 'Research Public Policy',
      url: 'ResearchPublicPolicy11.jpg',
      catNum: '11',
      photoCat: 'Government',
    },
  ];
  ...
  var states = [
    ...
    { state: 'newjersey', abbr: 'NJ' },
    { state: 'newmexico', abbr: 'NM' },
    { state: 'newyork', abbr: 'NY' },
    { state: 'northcarolina', abbr: 'NC' },
    { state: 'NorthDakota', abbr: 'ND' },
    { state: 'ohio', abbr: 'OH' },
    { state: 'oklahoma', abbr: 'OK' },
    { state: 'oregon', abbr: 'OR' },
    { state: 'Pennsylvania', abbr: 'PA' },
    { state: 'RhodeIsland', abbr: 'RI' },
    { state: 'SouthCarolina', abbr: 'SC' },
    { state: 'SouthDakota', abbr: 'SD' },
    ...
    { state: 'WestVirginia', abbr: 'WV' },
    { state: 'wisconsin', abbr: 'WI' },
    { state: 'wyoming', abbr: 'WY' },
    { state: 'DistrictOfColumbia', abbr: 'DC' },
  ];
  ...
  setTimeout(function () {
    for (var i = 0; i < categoryImages.length; i++) {
      ...
      var catDiv = $(
        '<div class="col s6 m4 category" data="' +
          categoryImages[i].catNum +
          '">'
      );
      var cardA = $('<a href="results.html" target=""></a>');
      var cardDiv = $(
        '<div class="card small"  style="height: 10rem; border-radius: 5px; margin-left:auto;margin-right:auto;"></div>'
      );
      var cardImg = $(
        '<div class="card-image" style="max-height: 100%; overflow: none;"></div>'
      );
      cardImg.append('<img class="responsive-img" src="' + newPic + '">');
      cardImg.append(
        '<span class="card-title">' + categoryImages[i].category + '</span>'
      );
  ...
  $('#resultsDiv img').attr(
    'src',
    localStorage.getItem('catPic' + localStorage.getItem('category'))
  );
    ...
    function (childSnapshot) {
      ...
      var $npDiv = $(
        '<div id="indexFav-' +
          ...
          childSnapshot.val().FavCharity +
          '"></div>'
      );

      //Card Div
      var $mycard = $(
        '<div class="card" style="height:275px; border-radius: 7px;"></div>'
      );
      ...
      var $cardContentAction = $(
        '<div class="card-action" style="padding: 5px 18px;overflow:auto;">'
      );

      var $cardContentSpan = $(
        '<p class="card-title activator grey-text text-darken-4" style="font-size:18px;line-height:1.3rem;"></p>'
      );
      ...
      var $cardRatingImg = $(
        '<img src="' + childSnapshot.val().CharRating + '" class="img-rating">'
      );
      ...
      var $cardRevealLink1 = $(
        '<a href="' + childSnapshot.val().CharUrl + '" target="_blank">'
      );
      $cardRevealLink1.text('Contribute');
      $cardRevealP2.append($cardRevealLink1);
      var $cardRevealP3 = $('<p>');
      var $cardRevealBtn = $(
        '<button class="checkbox card-reveal-favorite-button" data="' +
          lengthFavList +
          '"></button>"'
      );
  ...
  function callCharApi(cat) {
    ...
    if (localStorage.getItem('category')) {
      var queryURL =
        'https://api.data.charitynavigator.org/v2/Organizations?&app_id=d555fab3&app_key=21adfc6c878ba1839bb8e6a8e0838951&pageSize=9&rated=true&state=' +
        state +
        '&categoryID=' +
        cat;
    }
  ...
  $('.char-card').on('click', '.cardcol', function () {
    ...
    document.getElementById('charityMap').src =
      'https://www.google.com/maps/embed/v1/search?key=AIzaSyBIPIXu1T9Mi7cPAIYSlDiwaXMKWAxeB48&zoom=10&q=' +
      newURL;
  });
  ...
  $('.char-card').on('click', '.favChar', function () {
    ...
    var copyTag = $(
      '#index-' + nameClicked + ' .card .card-action .card-title span'
    ).text();
    console.log(copyName);
    var copyPic = $('#index-' + nameClicked + ' .card .card-image img').attr(
      'src'
    );
    var copyRating = $('#index-' + nameClicked + ' .card .img-rating').attr(
      'src'
    );
    var copyStat = $(
      '#index-' + nameClicked + ' .card .card-reveal :nth-child(2)'
    ).text();
    var copyUrl = $('#index-' + nameClicked + ' .card .card-reveal p a').attr(
      'href'
    );
